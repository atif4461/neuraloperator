...
def load_dpsl_2d_pt(
    ...
    x_train = torch.tensor(ftrain['vortz'][0:n_train,0,:,:]).unsqueeze(channel_dim).type(torch.float32).clone()
    y_train = torch.tensor(ftrain['vortz'][0:n_train,4,:,:]).unsqueeze(channel_dim).type(torch.float32).clone()
    # unsqueeze uplifts torch.Size([1000, 16, 16]) to torch.Size([1000, 1, 16, 16])
    # print(f"shapes {torch.tensor(ftrain['vortz'][0:n_train,0,::4,::4]).shape}, {x_train.shape}")
    ...
    x_test  = torch.tensor(ftrain['vortz'][-n_test:,0,:,:]).unsqueeze(channel_dim).type(torch.float32).clone()
    y_test  = torch.tensor(ftrain['vortz'][-n_test:,4,:,:]).unsqueeze(channel_dim).type(torch.float32).clone()
